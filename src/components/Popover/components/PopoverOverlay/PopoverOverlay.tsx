...
export default class PopoverOverlay extends React.PureComponent<Props, State> {
  ...
  render() {
    ...
    if (transitionStatus === 'exited' && !active) return null;

    const className = classNames(
      styles.PopoverOverlay,
      transitionStatus === 'entered' && styles['PopoverOverlay-open'],
      transitionStatus === 'exiting' && styles['PopoverOverlay-exiting'],
    );
  ...
  private handleClick = (event: Event) => {
    ...
    if (
      ...
      this.state.transitionStatus !== 'entered'
    ) {
      return;
    }
