...
We have enabled extending styles with styled-components and inline styles by accepting the className and style attributes in a series of components which are listed below. To do this, we add the following to a given component:
...
The last parameter of the `classNames()` function:
```
...extendedClasses.split(' '),
```

On markup tags which have the property `className={className}`:
```
style={style}
```
...
<link
  rel="stylesheet"
  href="https://unpkg.com/@shopify/polaris@4.9.0/styles.min.css"
/>
```
